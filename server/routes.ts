...
Provide only the translation.`;
  ...
  app.get('/api/study-resources/:id/export', mockAuth, async (req: any, res) => {
    try {
      ...
      let filename = `${resource.title.replace(/[^a-zA-Z0-9]/g, '_')}.txt`;
      ...
      switch (format) {
        case 'pdf':
          ...
          // Generate simple PDF content (simplified)
...
(${resource.title}) Tj
0 -24 Td
(${resource.content.substring(0, 100)}...) Tj
ET
endstream
endobj
...
%%EOF`;
        ...
        case 'csv':
          contentType = 'text/csv';
          filename = `${resource.title.replace(/[^a-zA-Z0-9]/g, '_')}.csv`;
          ...
          content = `Title,Type,Content,Created At,Last Accessed,Access Count,Rating\n`;
          content += `${escapeCsv(resource.title)},${escapeCsv(resource.type)},${escapeCsv(resource.content)},${escapeCsv(resource.createdAt)},${escapeCsv(resource.lastAccessedAt || '')},${resource.accessCount || 0},${resource.rating || 0}`;
          break;
          
        case 'json':
          contentType = 'application/json';
          filename = `${resource.title.replace(/[^a-zA-Z0-9]/g, '_')}.json`;
          
          // Clean up the resource object for export
          const exportResource = {
            ...
            isAutoGenerated: resource.isAutoGenerated,
            tags: resource.tags || [],
            exportedAt: new Date().toISOString(),
            exportedBy: userId
          };
          content = JSON.stringify(exportResource, null, 2);
          break;
        ...
        default: // text format
          ...
          content += `Created: ${new Date(resource.createdAt).toLocaleString()}\n`;
          if (resource.lastAccessedAt) {
            content += `Last Accessed: ${new Date(resource.lastAccessedAt).toLocaleString()}\n`;
          }
          content += `Access Count: ${resource.accessCount || 0}\n`;
          content += `Rating: ${resource.rating || 0}/5\n\n`;
          content += `Content:\n`;
          content += `${'-'.repeat(8)}\n`;
          content += `${resource.content}\n\n`;
  ...
  app.post('/api/study-resources/bulk-export', mockAuth, async (req: any, res) => {
    try {
      ...
      let filename = `study-resources-bulk-${timestamp}.txt`;
      ...
      switch (format) {
        case 'pdf':
          contentType = 'application/pdf';
          filename = `study-resources-bulk-${timestamp}.pdf`;
          
          // Generate a basic PDF structure with multiple resources
          content = `%PDF-1.4\n`;
          content += `1 0 obj\n<<\n/Type /Catalog\n/Pages 2 0 R\n>>\nendobj\n\n`;
          content += `2 0 obj\n<<\n/Type /Pages\n/Kids [3 0 R]\n/Count 1\n>>\nendobj\n\n`;
          ...
          let pdfText = 'BT\n/F1 12 Tf\n72 720 Td\n';
          pdfText += `(Study Resources Export - ${selectedResources.length} items) Tj\n0 -24 Td\n`;
          
          selectedResources.forEach((r, index) => {
            pdfText += `(${index + 1}. ${r.title}) Tj\n0 -18 Td\n`;
            const shortContent = r.content.substring(0, 100).replace(/[()]/g, '');
            pdfText += `(${shortContent}...) Tj\n0 -36 Td\n`;
          });
        ...
        case 'csv':
          contentType = 'text/csv';
          filename = `study-resources-bulk-${timestamp}.csv`;
          
          content = 'Title,Type,Content,Created At,Last Accessed,Access Count,Rating\n';
          content += selectedResources.map(r => 
            `${escapeCsv(r.title)},${escapeCsv(r.type)},${escapeCsv(r.content)},${escapeCsv(r.createdAt)},${escapeCsv(r.lastAccessedAt || '')},${r.accessCount || 0},${r.rating || 0}`
          ).join('\n');
          break;
          
        case 'json':
          contentType = 'application/json';
          filename = `study-resources-bulk-${timestamp}.json`;
        ...
        default: // text format
          filename = `study-resources-bulk-${timestamp}.txt`;
          
          content = `Study Resources Export\n`;
          content += `${'='.repeat(25)}\n\n`;
          content += `Exported: ${new Date().toLocaleString()}\n`;
          content += `Total Resources: ${selectedResources.length}\n\n`;
          
          selectedResources.forEach((r, index) => {
            content += `${index + 1}. ${r.title}\n`;
            content += `${'='.repeat(r.title.length + 3)}\n`;
            content += `Type: ${r.type}\n`;
            content += `Created: ${new Date(r.createdAt).toLocaleString()}\n`;
            if (r.lastAccessedAt) {
              content += `Last Accessed: ${new Date(r.lastAccessedAt).toLocaleString()}\n`;
            }
            content += `Access Count: ${r.accessCount || 0}\n`;
            content += `Rating: ${r.rating || 0}/5\n\n`;
            content += `Content:\n${'-'.repeat(8)}\n${r.content}\n\n`;
          ...
          content += `End of Export - ${selectedResources.length} resources`;
      }
