...
Return the response as valid JSON with all the above sections.`;
      ...
      // Use existing AI service with proper parameters
      const generatedQuestions = await generateQuestionsWithAI({
        topic: `CAT Exam Configuration: ${title}`,
        questionCount: 1, // We're generating 1 configuration object, not questions
        questionTypes: ['multiple_choice'], // Required parameter
        difficultyRange: { min: 3, max: 8 }, // Required parameter
        bloomsLevels: ['apply'], // Required parameter
        includeReferences: false, // Required parameter
        referenceLinks: [], // Required parameter
        targetAudience: 'Educators', // Required parameter
        learningObjectives: ['Generate CAT exam configuration'], // Required parameter
        questionStyles: ['educational'], // Required parameter
        includeImages: false, // Required parameter
        includeMultimedia: false, // Required parameter
        customInstructions: aiPrompt // Pass our actual prompt here
      }, (progress) => {
        // Progress callback - just log for now
        console.log(`AI Generation Progress: ${progress.status} (${progress.current}/${progress.total})`);
      });

      // The AI service returns an array, but we expect a configuration object
      // Extract the configuration from the response
      let examConfig;
      if (generatedQuestions && generatedQuestions.length > 0) {
        // Try to parse the first result as our configuration
        const firstResult = generatedQuestions[0];
        if (typeof firstResult === 'object' && firstResult !== null) {
          examConfig = firstResult;
        } else {
          // If it's not an object, it might be a string we need to parse
          try {
            examConfig = typeof firstResult === 'string' ? JSON.parse(firstResult) : firstResult;
          } catch (parseError) {
        ...
        return res.status(500).json({ message: 'No configuration generated by AI' });
      }
